...
echo "début du tableaux<br>";
print_r ($_POST);
echo "<br>fin du tableaux<br>";
...
if (isset($_POST['ele_politesse']) or isset($_POST['rep_politesse']) or isset($_POST['id_codebarre']) or isset($_POST['id_codebarre']))
{	//echo return_array($_POST, true );
	...
	{ 	
		...
		$tabStudent=$bd->GetStudent( $stu );
		$OnlyStudent =$bd-> GetStudentOnly($stu);
		//echo json_encode($tabStudent[0]);
		...
		*/
		foreach($_POST as $key => $val) {
			...
			if(array_key_exists( $key, $tabStudent[0] ))
			{
				...
				if ($tabStudent[0][$key] != $val) {
						$i++;
                        echo gettype($replegal);
						print_r($replegal);
						echo "<br>la valeur ".$key;
						if(in_array( $key, $replegal ))
						{
                            echo "j'update un represantant legal ";
						    if ($rlgl == 0 )
								{
									echo "0rlgl0<br>";
									...
									$sql='Update t_representantlegal set  '.$key.'="'.$val.'" where id_representantlegal='.$rlgl.'';
						...
						} else if(array_key_exists( $key, $tabStudent[0] ))
						{
						    ...
						    echo "j'update un eleve ";
						    //Check is date
								if (preg_match("/[0-9]{2}\.[0-9]{2}\.[0-9]{4}/", $val)){
                                    $val = date('Y-m-d',strtotime($val));
                                    echo $val." je suis la date";
                                }
