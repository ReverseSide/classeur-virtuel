...
$corpsmail="<p>Madame, Monsieur<br><br>
                                ...
                                Nous insistons sur le fait que votre présence est obligatoire. Vous serez convoqué au Conseil de discipline si vous ne vous présentez pas à ce rendez-vous.<br><br>
...
$requete5=$bdd->prepare("SELECT count(id_absence)as 'nbrperiode',id_eleve,ele_nom,ele_prenom,cla_nom,abs_date,abs_excuse,ele_mail,idx_entreprise FROM t_absence,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_absence.idx_eleve=t_eleve.id_eleve AND t_absence.idx_cours=t_sdh.id_sdh AND t_absence.idx_classe=t_classe.id_classe  AND t_sdh.cou_periode=t_horaire.id_horaire  AND (abs_traite='0' OR abs_traite IS NULL) and abs_excuse ='non'  and t_absence.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') group by abs_date,idx_eleve ORDER BY abs_date desc ");
$requete5->execute();
$nbperiode=$requete5->fetchAll();


$requete4=$bdd->prepare("SELECT *	 FROM t_absence,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_absence.idx_eleve=t_eleve.id_eleve AND t_absence.idx_cours=t_sdh.id_sdh AND t_absence.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND (abs_traite='0' OR abs_traite IS NULL) and abs_excuse ='non'  and t_absence.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') ORDER BY abs_date desc");
$requete4->execute();
...
<div class="col-xs-12">
    <div class="col-lg-8 col-sm-8 col-xs-12 no-padding edusecArLangCss"><h3 class="box-title"><i class="fa fa-th-list"></i> Absences non excusées à traiter</h3></div>
    <div class="col-lg-4 col-sm-4 col-xs-12 no-padding" style="padding-top: 20px !important;">
	  ...
	  <a class="btn btn-block btn-primary"  href="absences_excuse.php?dpt= <?php echo $_GET['dpt'];?>">Absences Excusées</a>    </div>
</div>
</div>
 ...
 else if($difference == -1)
 {
    ...
    echo "
            ...
            <div class='panel panel-default'>
				...
				<form name='". $rs['id_eleve'] .$rs['abs_date']."'  method='get'>
					<table style='table-layout:fixed; width: 620px;'>
					<tr>
					<td data-toggle='collapse' data-parent='#accordion' href='#". $rs['id_eleve'] .$rs['abs_date']."' style='width: 245px;  margin: auto; border-collapse: separate;'>"  . $rs['ele_nom'] ." ". $rs['ele_prenom'] ." 
					</td>
					<td data-toggle='collapse' data-parent='#accordion' href='#". $rs['id_eleve'] .$rs['abs_date']."' style='width: 175px;  margin: auto; border-collapse: separate;'>" . $rs['cla_nom'] ."</td>
					<td data-toggle='collapse' data-parent='#accordion' href='#". $rs['id_eleve'] .$rs['abs_date']."' style='width: 145px;  margin: auto; border-collapse: separate;'> Nb de périodes :" . $rs['nbrperiode'] ."</td>
			...
			echo"		<td style='width: 15px;  margin: auto;border-collapse: separate;'></td>
				<td style='width: 125px;  margin: auto;border-collapse: separate;'>
					<a width: '100px' type='submit' class='btn btn-primary'   href='absences_trt.php?traitet=Oui&eleve=".$rs['id_eleve']."&date=".$rs['abs_date']."&dpt=".$departement."'  style='text-align: right;'>A traiter</button></td>
					<td style='width: 125px; border-collapse: separate;'>" .date("d/m/Y", strtotime($rs['abs_date']))."</td>
					<td style='width: 25px; border-collapse: separate;'><input type='checkbox' name='ckbabsence[]' value='id:". $rs['id_eleve']." | Nom: " .$rs['ele_nom']." " .$rs['ele_prenom']. " | Classe: " .$rs['cla_nom']. "| Nbr cas: " .$rs['nbrperiode'].  " | Date:" .$rs['abs_date']. $mail_ent." | Mail: ".$rs['ele_mail']."'>
</div></td>
					</tr>
                ...
                <div id='". $rs['id_eleve'] .$rs['abs_date']."' class='panel-collapse collapse'>
                    <div class='panel-body'>
                        <p><div id='w0' class='grid-view'>
		<table class='table table-striped table-bordered'><thead>

<tbody>";
						$requete4=$bdd->prepare("SELECT *, DATE_FORMAT(abs_date,'%d/%l/%Y') AS 'dateabs' ,t_professeur.pro_nom,t_professeur.pro_prenom FROM t_absence,t_eleve,t_sdh,t_classe,t_horaire  ,t_professeur  WHERE t_professeur.id_professeur = t_absence.idx_professeur and  t_absence.idx_eleve=t_eleve.id_eleve AND t_absence.idx_cours=t_sdh.id_sdh AND t_absence.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND abs_traite='0' and t_absence.idx_eleve='". $rs['id_eleve']."' and t_absence.abs_date='". $rs['abs_date']."'  ORDER BY abs_date,ele_nom ASC");
$requete4->execute();
$absence=$requete4->fetchAll();
	$i=1;					
...
<td width='30'><small style='cursor:pointer;' data-toggle='tooltip' data-placement='bottom' data-html='true'
                               title='Remarque: ".$rs2['abs_commentaire']."'>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Absences non traitées à traiter: Mailing</h4>
        </div>
		<br>
