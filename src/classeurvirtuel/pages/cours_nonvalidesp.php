...
$startDate=time();
$finishdate=strtotime("2016-08-22");
    ...
    foreach ($classe as $classeencours)
    {
        ...
        foreach(array_reverse($tabSchedule) as $entry)
        {
            ...
            //{
                ...
                $requete=$bdd->prepare("SELECT id_presprof,t_professeur.pro_nom ,  t_professeur.pro_prenom FROM t_presprof,t_professeur,t_sdh WHERE t_professeur.id_professeur = t_sdh.idx_professeur and pre_date ='".$dateatester."' AND idx_cours ='".$entry['id_sdh']."';");
                $requete->execute();
                ...
                else
                {
					...
					$nomprof= $entry['pro_nom'] . " " .$entry['pro_prenom'] ;
            ...
            <div class='panel panel-default'>
                <div class='panel-heading'>
					...
					<tr>
					<td data-toggle='collapse' data-parent='#accordion' style='width: 100px;  margin: auto; border-collapse: separate;'>". $nomclasse ." 
					</td>
					<td data-toggle='collapse' data-parent='#accordion' style='width: 200px;  margin: auto; border-collapse: separate;'>" .$nomcours ."</td>
					
					<td data-toggle='collapse' data-parent='#accordion' style='width: 200px;  margin: auto; border-collapse: separate;'>  ". $nomprof ." 
					</td>
					...
					<td data-toggle='collapse' data-parent='#accordion' style='width: 160px;  margin: auto; border-collapse: separate;'> Date : " . date('d/m/Y',$startDate) ."</td>
					<td style='width: 100px;  margin: auto;border-collapse: separate;'>  
					     

					<a class='btn btn-block btn-warning'  href='cours_nonvalides.php?dpt=".$departement."&date=".$dateatester."&cours=".$entry['id_sdh']."' >Valider</a></td></tr></table></h4></div></div>";
