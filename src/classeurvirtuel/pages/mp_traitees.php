...
$sql=" update t_porte set por_traite='0' where idx_eleve='".$_GET['eleve']."' and por_date='".$_GET['date']."'";
...
$change->execute();
	...
	if(isset($_GET['Traite'])=='Oui'){
		 $sql=" update t_porte set por_traite='0' where id_porte='".$_GET['idabs']."'";
            $change=$bdd->prepare($sql);
            $change->execute();
	}
...
$requete5=$bdd->prepare("SELECT count(id_porte)as 'nbrperiode',id_eleve,ele_nom,ele_prenom,cla_nom,por_date,ele_mail,idx_entreprise FROM t_porte,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_porte.idx_eleve=t_eleve.id_eleve AND t_porte.idx_cours=t_sdh.id_sdh AND t_porte.idx_classe=t_classe.id_classe  AND t_sdh.cou_periode=t_horaire.id_horaire  AND por_traite='1'  and t_porte.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') group by por_date,idx_eleve ORDER BY por_date desc ");
$requete5->execute();
$nbperiode=$requete5->fetchAll();


$requete4=$bdd->prepare("SELECT *	 FROM t_porte,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_porte.idx_eleve=t_eleve.id_eleve AND t_porte.idx_cours=t_sdh.id_sdh AND t_porte.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND por_traite='1'  and t_porte.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') ORDER BY por_date desc");
$requete4->execute();
...
<div class="col-xs-12">
    <div class="col-lg-8 col-sm-8 col-xs-12 no-padding edusecArLangCss"><h3 class="box-title"><i class="fa fa-th-list"></i> Mises à la porte traitées</h3></div>
    ...
    echo "
		 ...
		 if (isset( $entreprise[0]['ent_mail'])){  
			...
			echo"		<td style='width: 15px;  margin: auto;border-collapse: separate;'></td>
				<td style='width: 125px;  margin: auto;border-collapse: separate;'>
					<a width: '100px' type='submit' class='btn btn-primary'   href='mp_trt.php?traitet=Oui&eleve=".$rs['id_eleve']."&date=".$rs['por_date']."&dpt=".$departement."'  style='text-align: right;'>En instance</button></td>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Mises à la porte: Mailing</h4>
        </div>
		<br>
