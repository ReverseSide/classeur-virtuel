...
$sql=" update t_porte set por_traite='1' where idx_eleve='".$_GET['eleve']."' and por_date='".$_GET['date']."'";
...
$change->execute();
	...
	if(isset($_GET['Traite'])=='Oui'){
		 $sql=" update t_porte set por_traite='1' where id_porte='".$_GET['idabs']."'";
            $change=$bdd->prepare($sql);
            $change->execute();
	}
...
$requete5=$bdd->prepare("SELECT count(id_porte)as 'nbrperiode',id_eleve,ele_nom,ele_prenom,cla_nom,por_date,ele_mail,idx_entreprise FROM t_porte,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_porte.idx_eleve=t_eleve.id_eleve AND t_porte.idx_cours=t_sdh.id_sdh AND t_porte.idx_classe=t_classe.id_classe  AND t_sdh.cou_periode=t_horaire.id_horaire  AND (por_traite='0' OR por_traite IS NULL)  and t_porte.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') group by por_date,idx_eleve ORDER BY por_date desc ");
$requete5->execute();
$nbperiode=$requete5->fetchAll();


$requete4=$bdd->prepare("SELECT *	 FROM t_porte,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_porte.idx_eleve=t_eleve.id_eleve AND t_porte.idx_cours=t_sdh.id_sdh AND t_porte.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND (por_traite='0' OR por_traite IS NULL)  and t_porte.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') ORDER BY por_date desc");
$requete4->execute();
...
<div class="col-xs-12">
    <div class="col-lg-8 col-sm-8 col-xs-12 no-padding edusecArLangCss"><h3 class="box-title"><i class="fa fa-th-list"></i> Mises à la porte à traiter</h3></div>
    <div class="col-lg-4 col-sm-4 col-xs-12 no-padding" style="padding-top: 20px !important;">
	  ...
	  <a class="btn btn-block btn-primary"  href="mp_traitees.php?dpt= <?php echo $_GET['dpt'];?>">Cas traités</a>    </div>
</div>
</div>
 ...
 else if($difference == -1)
 {
    ...
    echo "
            ...
            <div class='panel panel-default'>
				...
				<form name='". $rs['id_eleve'] .$rs['por_date']."'  method='get'>
					<table style='table-layout:fixed; width: 620px;'>
					<tr>
					<td data-toggle='collapse' data-parent='#accordion' href='#". $rs['id_eleve'] .$rs['por_date']."' style='width: 245px;  margin: auto; border-collapse: separate;'>"  . $rs['ele_nom'] ." ". $rs['ele_prenom'] ." 
					</td>
					<td data-toggle='collapse' data-parent='#accordion' href='#". $rs['id_eleve'] .$rs['por_date']."' style='width: 175px;  margin: auto; border-collapse: separate;'>" . $rs['cla_nom'] ."</td>
					<td data-toggle='collapse' data-parent='#accordion' href='#". $rs['id_eleve'] .$rs['por_date']."' style='width: 145px;  margin: auto; border-collapse: separate;'> Nb de périodes :" . $rs['nbrperiode'] ."</td>
		 ...
		 if (isset( $entreprise[0]['ent_mail'])){  
			...
			echo"		<td style='width: 15px;  margin: auto;border-collapse: separate;'></td>
				<td style='width: 125px;  margin: auto;border-collapse: separate;'>
					<a width: '100px' type='submit' class='btn btn-primary'   href='mp_trt.php?traitet=Oui&eleve=".$rs['id_eleve']."&date=".$rs['por_date']."&dpt=".$departement."'  style='text-align: right;'>A traiter</button></td>
					<td style='width: 125px; border-collapse: separate;'>" .date("d/m/Y", strtotime($rs['por_date']))."</td>
					<td style='width: 25px; border-collapse: separate;'><input type='checkbox' name='ckbabsence[]' value='id:". $rs['id_eleve']." | Nom: " .$rs['ele_nom']." " .$rs['ele_prenom']. " | Classe: " .$rs['cla_nom']. "| Nbr cas: " .$rs['nbrperiode'].  " | Date:" .$rs['por_date']. $mail_ent." | Mail: ".$rs['ele_mail']."'>
</div></td>
					</tr>
                ...
                <div id='". $rs['id_eleve'] .$rs['por_date']."' class='panel-collapse collapse'>
                    <div class='panel-body'>
                        <p><div id='w0' class='grid-view'>
		<table class='table table-striped table-bordered'><thead>

<tbody>";
						$requete4=$bdd->prepare("SELECT *, DATE_FORMAT(por_date,'%d/%l/%Y') AS 'dateabs',t_professeur.pro_nom,t_professeur.pro_prenom FROM t_porte,t_eleve,t_sdh,t_classe,t_horaire   ,t_professeur  WHERE t_professeur.id_professeur = t_porte.idx_professeur and t_porte.idx_eleve=t_eleve.id_eleve AND t_porte.idx_cours=t_sdh.id_sdh AND t_porte.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND por_traite='0' and t_porte.idx_eleve='". $rs['id_eleve']."' and t_porte.por_date='". $rs['por_date']."'  ORDER BY por_date,ele_nom ASC");
$requete4->execute();
$absence=$requete4->fetchAll();
	$i=1;					
...
<td width='30'><small style='cursor:pointer;' data-toggle='tooltip' data-placement='bottom' data-html='true'
                               title='Remarque: ".$rs2['por_commentaire']."'>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Mises à la porte à traiter: Mailing</h4>
        </div>
		<br>
        ...
        <div class="modal-body">
		  ...
		  <form name="frmAddConvocation" id="frmAddConvocation" action="./chkForm/chkmailmp.php" method="POST">							
						...
						<textarea width="700" height="700" id="txtcorps" name="txtcorps" rows="60*60*24" ></textarea>
						<br /><br />
					</div>
					</div>
		  ...
		  <div class="row">
                                                <div class="col-xs-10">
													...
													</label>     <input type='checkbox' name='ckbcopiepatron' value='<?php if (isset($rs['id_eleve'])) echo $rs['id_eleve'].$rs['por_date'] ;?>."'>
                                                   <label for="cbox2">
												Traiter les cas
													</label>     <input type='checkbox' name='ckbtraiter' value='<?php  if (isset($rs['id_eleve'])) echo $rs['id_eleve'].$rs['por_date'] ;?>."'>
