...
$sql=" update t_tardive set tar_traite='0' where idx_eleve='".$_GET['eleve']."' and tar_date='".$_GET['date']."'";
...
$change->execute();
	...
	if(isset($_GET['Traite'])=='Oui'){
		 $sql=" update t_tardive set tar_traite='0' where id_tardive='".$_GET['idabs']."'";
            $change=$bdd->prepare($sql);
            $change->execute();
	}
...
$requete5=$bdd->prepare("SELECT count(id_tardive)as 'nbrperiode',id_eleve,ele_nom,ele_prenom,cla_nom,tar_date,ele_mail,idx_entreprise FROM t_tardive,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_tardive.idx_eleve=t_eleve.id_eleve AND t_tardive.idx_cours=t_sdh.id_sdh AND t_tardive.idx_classe=t_classe.id_classe  AND t_sdh.cou_periode=t_horaire.id_horaire  AND tar_traite='1'  and t_tardive.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') group by tar_date,idx_eleve ORDER BY tar_date desc ");
$requete5->execute();
$nbperiode=$requete5->fetchAll();


$requete4=$bdd->prepare("SELECT *	 FROM t_tardive,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_tardive.idx_eleve=t_eleve.id_eleve AND t_tardive.idx_cours=t_sdh.id_sdh AND t_tardive.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND tar_traite='1'  and t_tardive.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') ORDER BY tar_date desc");
$requete4->execute();
...
<div class="col-xs-12">
    <div class="col-lg-8 col-sm-8 col-xs-12 no-padding edusecArLangCss"><h3 class="box-title"><i class="fa fa-th-list"></i> Retards traités</h3></div>
    <div class="col-lg-4 col-sm-4 col-xs-12 no-padding" style="padding-top: 20px !important;">
	  ...
	  <a class="btn btn-block btn-primary"  href="retard_trt.php?dpt= <?php echo $_GET['dpt'];?>">Retour</a>    </div>
</div>
</div>
 ...
 else if($difference == -1)
 {
    ...
    echo "
			...
			echo"		<td style='width: 15px;  margin: auto;border-collapse: separate;'></td>
				<td style='width: 125px;  margin: auto;border-collapse: separate;'>
					<a width: '100px' type='submit' class='btn btn-primary'   href='retard_trt.php?traitet=Oui&eleve=".$rs['id_eleve']."&date=".$rs['tar_date']."&dpt=".$departement."'  style='text-align: right;'>En instance</button></td>
...
<tbody>";
						$requete4=$bdd->prepare("SELECT *, DATE_FORMAT(tar_date,'%d/%l/%Y') AS 'dateabs',t_professeur.pro_nom,t_professeur.pro_prenom FROM t_tardive,t_eleve,t_sdh,t_classe,t_horaire ,t_professeur  WHERE t_professeur.id_professeur = t_tardive.idx_professeur and   t_tardive.idx_eleve=t_eleve.id_eleve AND t_tardive.idx_cours=t_sdh.id_sdh AND t_tardive.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND tar_traite='1' and t_tardive.idx_eleve='". $rs['id_eleve']."' and t_tardive.tar_date='". $rs['tar_date']."'  ORDER BY tar_date,ele_nom ASC");
$requete4->execute();
$absence=$requete4->fetchAll();
	$i=1;					
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Retards: Mailing</h4>
        </div>
		<br>
