...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                $filename="semaine_sportive_ete.csv";
               // $req=$bdd->prepare("SELECT * FROM t_eleve INNER JOIN t_sporthiver ON t_sporthiver.id_eleve=t_eleve.id_eleve JOIN t_classe ON t_classe.id_classe = t_eleve.idx_classe ORDER BY t_eleve.id_eleve");
                    $req=$bdd->prepare("SELECT ele_prenom as prenom, ele_nom as nom, ele_datedenaissance as Date_de_naissance,codeBarre as code_barre, (select cla_nom from t_classe where id_classe=t_eleve.idx_classe) as nom_classe,(select cla_joursSport from t_classe where id_classe=t_eleve.idx_classe) as jours_de_cours, choixSport as sport, ele_Rue as Rue, ele_npa as NPA, ele_localite as Ville, ele_numeromobile as Téléphone, arrivee as heure_arrivee, depart as heure_depart from t_sportete, t_eleve where t_sportete.id_eleve = t_eleve.id_eleve order by t_eleve.id_eleve");
                    $req->execute();
                ...
                header("Content-Disposition: attachment; filename=semaine_sportive_ete".$date.".zip");
                die($zipContent);
