	...
	} else if($difference == -1)
	{
		$rs['dateabs']  = 'Hier';
			...
			echo"		<td style='width: 15px;  margin: auto;border-collapse: separate;'></td>
				<td style='width: 125px;  margin: auto;border-collapse: separate;'>
					<a width: '100px' type='submit' class='btn btn-primary'   href='absences_trt.php?traitet=Oui&eleve=".$rs['id_eleve']."&date=".$rs['abs_date']."&dpt=".$departement."'  style='text-align: right;'>En Instance</button></td>
...
<tbody>";
	$requete4=$bdd->prepare("SELECT *, DATE_FORMAT(abs_date,'%d/%l/%Y') AS 'dateabs' ,t_professeur.pro_nom,t_professeur.pro_prenom FROM t_absence,t_eleve,t_sdh,t_classe,t_horaire  ,t_professeur  WHERE t_professeur.id_professeur = t_absence.idx_professeur and  t_absence.idx_eleve=t_eleve.id_eleve AND t_absence.idx_cours=t_sdh.id_sdh AND t_absence.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND abs_traite='1' and t_absence.idx_eleve='". $rs['id_eleve']."' and t_absence.abs_date='". $rs['abs_date']."'  ORDER BY abs_date,ele_nom ASC");
	$requete4->execute();
	$absence=$requete4->fetchAll();
	$i=1;					
