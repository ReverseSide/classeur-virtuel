...
$Coursid=$requete6->fetchAll();
        ...
        // Fixe les doublons
        header("Refresh:0; url='student_dtl.php?stu=".$_GET['stu']."'");
        exit;
...
if(isset($_POST['dateRupture'])){
    ...
    if ( !isset( $_POST['ChoixSuivreCours'] )  && !isset( $_POST['dateRupture'] )  && !isset( $_GET['stu'] ) )
		...
		if( $choixSuivreCours == "Oui" || $choixSuivreCours == "Non" ){
			...
			$eleve="INSERT INTO `t_rupture` ( `rup_date`, `idx_codebarre`, `rup_suit`, `rup_modifieur`) VALUES
...
//Return si l'utilisateur courant est admin ou pas
$UserAdmin = $bd->IsAdmin($_SESSION['user_id']);
...
foreach($tabStudent as $entry)
{
	...
	$cla_dep_idx=$entry['idx_departement'];
	$idx_maitreClasse=$entry['idx_maitre_classe'];

	
	//*
	$reqM=$bdd->prepare("SELECT pro_nom, pro_prenom FROM t_professeur WHERE id_professeur='" . $idx_maitreClasse . "'");
	$reqM->execute();
	$maitreDeClasse=$reqM->fetch();

	//*
	$reqClass=$bdd->prepare("SELECT dep_nom FROM t_departement WHERE id_departement='".$cla_dep_idx."'");
	$reqClass->execute();
	$resDepClass=$reqClass->fetch();
	
	$ele_nContrat=$entry['ele_numerodecontrat'];
...
//KG Get classe
$requete3=$bdd->prepare("SELECT * FROM t_classe, t_typeformation WHERE id_classe='".$idx_classe_ele."' AND idx_typeformation=id_typeformation ");
$requete3->execute();
...
//séléction de coches
$requete3=$bdd->prepare("SELECT id_coche FROM t_coche WHERE idx_eleve='".$id_eleve."'");
$requete3->execute();
$tabStudentCoche=$requete3->fetchAll(); 
...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        ...
        <div class="row">
			...
			<button type="button" class="btn btn-primary btn-md  pull-right" onclick="window.history.back()" style="margin-top:5px" >Retour</button>
                ...
                <table class="table table-striped">
					...
					<?php if ( $av_exist == 0 )
					{
					?><?php if($UserAdmin==true){ ?><tr>
                        <th>Photo </th>
                        <td>
						<form action="student_dtl.php" method="POST" enctype="multipart/form-data" class="idealforms">
						<input  name="stu" type="hidden"  value='<?php echo $id_codebarre  ;?>'>
						<input  name="avatar" type="file"  value=''>
						<input type="hidden" name="MAX_FILE_SIZE" value="1234567" /><br><input type="submit" value="Modifier">
						</form>
						</td>
                    </tr><?php } ?>
					<?php 
					...
					<tr>
                        <th>Id système</th>
                        <td><?php echo $id_eleve;?></td>
                    </tr>
					<tr>
                        <th>Contrat - N°</th>
                        <td><?php echo $ele_nContrat;?></td>
                    </tr>
					<tr>
                        <th>Nom</th>
					...
					<tr>
                        <th>Département</th>
                        <td><?php echo $resDepClass['dep_nom'];?></td>
                    </tr>
                    <tr>
                        <th>Classe</th>
                        <td>
						...
						//séléction des élèves
						$idCT=$detclasse['0']['id_classe'];
						$requete3=$bdd->prepare("SELECT id_classe, cla_nom FROM t_classe");
						$requete3->execute();
						$StudentClasseAChange=$requete3->fetchAll();

						//if classe modifiée
						if (isset($_POST['ClasseAChangePost']))
						{
							$cla1 = $_POST['classe1'];


								$sql = "UPDATE `t_eleve` SET `idx_classe`=$cla1 WHERE `id_eleve`=$id_eleve";
						...
						foreach($StudentClasseAChange as $S)
						{
							if($S['id_classe'] == $detclasse['0']['id_classe'])
							{
								echo '<option selected value="' . $S['id_classe'] . '" >' . $S['cla_nom'] . '</option>';
							}
							else
							{
								echo '<option value="' . $S['id_classe'] . '" >' . $S['cla_nom'] . '</option>';
							}
						}
						echo '</select></div><input class="btn" type="submit" name="ClasseAChangePost" value="Enregistrer">';
						}
						else
						{
						echo $cla_nom;
						...
						?>
                            <a href="timeclock.php?idclasse=<?php echo $detclasse['0']['id_classe'];?>" class="btn btn-primary" >Aller à la classe</a>
						</td>
                    </tr>
                    <tr>
                        <th>Email</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ( @$rupture['0']['suit_cours'] == 'Oui' ) { 
									...
									echo	"<span class='label label-success'>Rupture</span>";
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            if($ele_datedenaissance != "" && validateDate($var)==true)
                            {
                                ...
                                {
                                    if ($ele_politesse =="M." OR $ele_politesse =="Monsieur") { echo "<span class='label label-danger'>Mineur</span>";} else { echo "<span class='label label-danger'>Mineure</span>";}

                                }
                                else
                                {
                                    if ($ele_politesse =="M." OR $ele_politesse =="Monsieur") { echo "<span class='label label-danger'>Majeur</span>";} else { echo "<span class='label label-danger'>Majeure</span>";}
					...
					<tr>
                        <th>Élève Statut</th>
                        <td><?php echo $ele_statut;?></td>
                    </tr>
					<tr>
                        <th>Élève Spécificités</th>
                        <td><?php echo "-";?></td>
                    </tr>
					<tr>
                        <th>Désavantage</th>
                        <td><?php echo $ele_desavantage;?></td>
                    </tr>
                    <tr>
                        <th>Dispenses</th>
                        <td>
					...
					<tr>
                        <th>Dérogation AT & DA</th>
                        <td><?php  echo $ele_derogation; ?></td>
                    </tr>
					<!-- <tr>
                        <th>Commentaires</th>
                        <td><?php echo "" ;?></td>
            ...
            <div class="col-lg-9 profile-data">
                <ul class="nav nav-tabs responsive " id="profileTab">
					...
					<li class="active" id="personal-tab"><a href="#personal" data-toggle="tab" class=""><img src="images\prive.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Prive</a>
                    <!-- <li id="address-tab"><a href="#address" data-toggle="tab"><i class="fa fa-home"></i> Adresse</a></li> -->
                    <li id="guardians-tab"><a href="#classe" data-toggle="tab"><img src="images\classes.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Classe</a></li>
                    <li id="guardians-tab"><a href="#guardians" data-toggle="tab"><img src="images\entreprise.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Entreprise</a></li>
                    <li id="guardians-tab"><a href="#stats" data-toggle="tab" onclick="ComputePresenceRatio('e', '<?= addslashes($_GET['stu']) ?>', document.getElementById('presence-ratio-placeholder'));"><img src="images\statistiques.jpg" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Statistiques</a></li>
					
					<!--DROPDOWN fait à la main à cause d'un bug -->
					<li class="dropdown">
					  <button onclick="myFunction()" class="dropbtn"><img src="images\details.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Détails
						<span class="caret"></span></button>
					  <div id="myDropdown" class="dropdown-content">
						<a href="#absences" data-toggle="tab"><img src="images\absences.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Absences</a>
						<a href="#retards" data-toggle="tab"><img src="images\arrTar.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Arrivées tardives</a>
						<a href="#mp" data-toggle="tab"><img src="images\misePorte.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Mises à la porte</a>
						<a href="#coches" data-toggle="tab"><img src="images\coche.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Coches</a>
					  </div>
					</li>
					<style>
					/* Dropdown Button */
					...
					<!--fin DROPDOWN fait main-->
					
                    <li id="guardians-tab"><a href="#remarques" data-toggle="tab"><img src="images\remarques.png" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Remarques</a></li>
                    <li id="guardians-tab"><a href="#eval" data-toggle="tab"><img src="images\eval.jpg" alt="?" style="float:left;width:25px;height:25px;">&nbsp;Notes</a></li>
                </ul>
                <div id="content" class="tab-content responsive ">
								...
								<div class="col-xs-12">
									<h2 class="page-header">
										...
										<div class="pull-right">
                                            ...
                                            <?php if($UserAdmin==true){ ?> <button class='btn btn-danger' type='button' data-toggle='modal' data-target='#ruptureEleve'>  En rupture</button> <?php } ?>
						    <?php
						    ///echo "array count :".count($Dispense).";<br>";
						    if ( !count($Dispense) ) {
						    ?>
                                            <?php if($UserAdmin==true){ ?> <button class='btn btn-warning' type='button' data-toggle='modal' data-target='#DispenseEleve'>  Dispense</button>  <?php } ?>
						    <?php }
							?>
								...
								<div class="col-xs-12">
									<div class="pull-left">
										<div class="econtainer" id="1editer" >
											<?php if($UserAdmin==true){ ?><button type="button" class="btn" onclick="myEdit('tal'); myEdit('tbl'); myEdit('tcl'); togl('1editer'); togl('1edition'); ">Éditer</button> <?php } ?>
										</div>
									...
									<form id="aForm" action="../api/set-student.php?<?php echo "stuid=".$_GET['stu']."&rlgl=".$idx_replegal; ?>" method="post">
										<table id="tal" border="1" class="table table-bordered table-striped">
						   ...
						   ?>
							...
							?>							
								   <div class="row">
									...
									<div class="row">						
										<div class="col-xs-12">
                                            <div class="econtainer" id="ed2ter" style="visibility:visible;">
                                                <button type="button" class="btn" onclick="myEdit('tbl');  togl('2editer'); togl('ed2ter');">Éditer</button>
                                            </div>
                                            <div class="econtainer" id="2editer" style="visibility:hidden;">
											...
											<form id="bForm" action="../api/set-student.php?<?php echo "stuid=".$_GET['stu']."&rlgl=".$idx_replegal; ?>" method="post">
												<table id="tbl" border="1" class="table table-bordered table-striped">
													<tr><th> - </th><th> - </th><th> - </th><th> - </th></tr>
												</table>
											</form>
										</div> <!---COL XS 12--->
							...
							<div class="row">						
								<div class="col-xs-12">
									...
									<form id="cForm" action="../api/set-student.php?<?php echo "stuid=".$_GET['stu']."&rlgl=".$idx_replegal; ?>" method="post">
									<br>
										<table id="tcl" border="1" class="table table-bordered table-striped">
											<tr><th> - </th><th> - </th><th> - </th><th> - </th></tr>
										</table>
									</form>
									<br><br>
                    ...
                    <div class="tab-pane" id="classe">
                        <div class="col-xs-12 ">
                            <h2 class="page-header">
								...
								<form action="#" method="POST">
								<!--<input type="submit" name="cocheCom" value="Submit">-->
								<input class="btn" type="submit" name="deleguePost" value="Enregistrer">
								<br>
								<br>
                                ...
                                <tbody>
                                <!--<tr>
                                    <td><b>Nombre de cours absent</b></td>
                                    <td>Nombre de cours absent</td>
									<td><b>Nombre d'arrivées tardives</b></td>
                                    <td>Nombre de cours absent</td>
                                </tr>-->
								...
								//séléction des élèves
								$idCT=$detclasse['0']['id_classe'];
								$requete3=$bdd->prepare("SELECT id_eleve, ele_nom, ele_prenom FROM t_eleve WHERE idx_classe=$idCT");
								$requete3->execute();
								$StudentClasse=$requete3->fetchAll(); 
								
								//if délégué modifié
								if (isset($_POST['deleguePost']) AND $_POST['deleg1'] != $_POST['deleg2'])
								{			
									$deleg1 = $_POST['deleg1'];
									$deleg2 = $_POST['deleg2'];
									
										
										$sql = "UPDATE `t_classe` SET `idx_delegue`=$deleg1,`idx_delegue2`=$deleg2 WHERE `id_classe`=$idCT";
                                ...
                                foreach($detclasse as $classe)
                                {
									...
									echo "<tr>";
                                    echo "<td><b>Departement</b></td><td>$depclasse</td>";
									echo "<td><b>Classe type</b></td><td>$classetype</td>";
									echo "</tr>";

									//2B
									echo "<tr>";
                                    $classenom=$classe['cla_nom'];
                                    echo "<td><b>Nom de la classe</b></td><td>$classenom</td>";
									echo "<td><b>Formation type</b></td><td>info</td>";
									echo "</tr>";
									
									//3C
									$classeniveau=$classe['cla_niveau'];
									echo "<tr>";
                                    echo "<td><b>Maître de classe</b></td><td>" . $maitreDeClasse['pro_prenom'] . " " . $maitreDeClasse['pro_nom'] . "</td>";
									echo "<td><b>Niveau de formation</b></td><td>$classeniveau</td>";
									echo "</tr>";

									//4D
									$classejdc=$classe['cla_jourdecours'];
									$tFormation=$classe['typ_nom'];
									echo "<tr>";
                                    echo "<td><b>Formation</b></td><td>$tFormation</td>";
									echo "<td><b>Horaire - Jour - ECG ou BT</b></td><td>$classejdc</td>";
									echo "</tr>";
									
									//5E
									echo "<tr>";
                                    echo "<td><b>Spécialisation</b></td><td>info</td>";
									echo "<td><b>Formation N° OFFT</b></td><td>info</td>";
									echo "</tr>";
									
									//6F
									echo "<tr>";
                                    echo "<td><b>Nombre d'élèves</b></td><td>".count($detnbeleve)."</td>";
									echo "<td><b> - </b></td><td> - </td>";
									echo "</tr>";
                                    
									//7G
									echo "<tr>";
                                    echo '<td><b>Délégué de classe (D1)</b></td>
										<td>
											<!--<div class="form-group">-->
												<select class="form-control" id="deleg1" name="deleg1">';
												
												
									foreach($StudentClasse as $S)
									{
										if($S['id_eleve'] == $detclasse['0']['idx_delegue'])
										{
											echo '<option selected value="' . $S['id_eleve'] . '" >' . $S['ele_nom'] . ' ' . $S['ele_prenom'] . '</option>';
										}
										else
										{
											echo '<option value="' . $S['id_eleve'] . '" >' . $S['ele_nom'] . ' ' . $S['ele_prenom'] . '</option>';
										}
									}
									...
									echo '<td><b>Remplaçant délégué (D2)</b></td>
										<td>
											<!--<div class="form-group">-->
												<select class="form-control" id="deleg2" name="deleg2">';
												
									foreach($StudentClasse as $S)
									{
										if($S['id_eleve'] == $detclasse['0']['idx_delegue2'])
										{
											echo '<option selected value="' . $S['id_eleve'] . '" >' . $S['ele_nom'] . ' ' . $S['ele_prenom'] . '</option>';
										}
										else
								...
								echo "<tr>";
                                echo "<td><b>Horaire - Salle - 1ère période</b></td><td>info</td>";
								echo "<td><b>Economat - Montant - ECG et BT</b></td><td>info</td>";
                                echo "</tr>
								</tbody>
								</table>";
                    ...
                    <div class="tab-pane" id="stats">
                        ...
                        <div class="row">
                            <table class="table table-bordered table-striped">
								...
								<tr>
                                    <td>Nombre de coches</td>
                                    <td><?= count($tabStudentCoche) ?></td>
                                </tr>
                                <tr>
                                    <td><b>Taux de présence</b></td>
                                    <td>
					...
					<h1>Tableau des arrivées tardives</h1>
						...
						<!--<div class="pull-left">
						 ...
						 <div class="entainer" id="4entainer" style="visibility:hidden;" >
							<form id="mForm" action="../api/set-ent.php?<?php echo "stuid=".$_GET['stu']."&entID=".$ent_id; ?>" method="post">
								<table id="tml" border="1" class="table table-bordered table-striped">										<tr><th> - </th><th> - </th><th> - </th><th> - </th></tr>
							</table></form>
						</div> -->
						...
						<div  class="econtainer" id="4AddMai" style="visibility:hidden;" >
							...
							<?php /*<!-- <div class="entainer" id="4mai" style="visibility:hidden;" >
								<form id="eForm" action="../api/set-ent.php?<?php echo "stuid=".$_GET['stu']."&entID=".$ent_id; ?>" method="post">
									<table id="tel" border="1" class="table table-bordered table-striped">
											<tr><th> - </th><th> - </th><th> - </th><th> - </th></tr>
									</table>
								</form>
							</div> --->*/ ?>
...
&nbsp;&nbsp;<i class="fa fa-info-circle"> </i> Entreprise<div class="pull-right"><button type="button" class="btn" data-toggle="modal" data-target="#ajout_maitre">Nouveau maître d'apprentissage</button>&nbsp;&nbsp;<button class="btn" type="button" data-toggle="modal" data-target="#ajout_ent">Nouvelle entreprise</button><br>
				...
				<div class="row">						
					<div class="col-xs-12">
						...
						<form id="mForm" action="../api/set-ent.php?<?php echo "stuid=".$_GET['stu']."&entID=".$ent_id; ?>" method="post">
							<table id="tml" border="1" class="table table-bordered table-striped">
                                <tr><th> - </th><th> - </th><th> - </th><th> - </th></tr>
							</table>
						</form>
...
<?php
        ...
        <div class="modal fade" id="DispenseEleve" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
					...
					<h1>Tableau des mises à la porte</h1>
                                ...
                                <div class="col-sm-10">
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <?php foreach($tabNotices as $entry): ?>
                                            <tr>
                                                <td><input type="text" class=" start-date" value="<?= $entry['rem_datedebut'] ?>" /></td>
                                                <td><input type="text" class=" end-date" value="<?= $entry['rem_datefin'] ?>" /></td>
					...
					<?php
					//séléction de coches
					$requete3=$bdd->prepare("SELECT id_coche, coc_commentaire, coc_creation, typ_nom FROM t_coche, t_typecoche WHERE idx_eleve='".$id_eleve."' AND idx_typecoche=id_typecoche");
					$requete3->execute();
					$StudentCocheRes=$requete3->fetchAll(); 
					?>
					
                    <div class="tab-pane" id="coches">
					
					<h1>Tableau des coches</h1>
						...
						<table class="table table-bordered table-striped">
                                <thead>
                                <tr>
								<th><i>Total de coches:</i> <?php echo count($StudentCocheRes); ?></th>
                                </tr>
                                </thead>
								</table>
						...
						<table class="table table-bordered table-striped">
								...
								<form action="#" method="POST">
								<!--<input type="submit" name="cocheCom" value="Submit">-->
								<input class="btn" type="submit" name="cocheCom" value="Enregistrer">
								<br>
                                <tbody>
								
                                <?php 
								
								$Yee=count($StudentCocheRes)-1;
								
								if (isset($_POST['cocheCom']))
								{
									
									for($i=0;$i<$_POST['P-incC'];$i++)
									{
										$idT = $StudentCocheRes[$i]['id_coche'];
										$newC = $_POST['comment-'.$i.''];
										
										
										$sql = "UPDATE `t_coche` SET `coc_commentaire`='$newC' WHERE `id_coche`=$idT";
								...
								foreach (array_reverse($StudentCocheRes) as $entry){
								
									$date = date_create($entry['coc_creation']);
									
									
									echo "<tr>";
									echo "<td>";
									echo date_format($date, 'd.m.Y');
									echo "</td><td>" . $entry['typ_nom'] . "</td>";
									echo "<td>";								
									echo '<input type="text" name="comment-' . $Yee . '" value="' . $entry['coc_commentaire'] . '">';
									echo "</td></tr>";
									$Yee--;
									$incC++;
								}
								echo '<input class="hidden" name="P-incC" value="' . $incC . '">';
                                ?>
	...
	<!-- Evaluation (Notes)-->
	<div class="tab-pane" id="eval" name="eval">
		<div class="row">
                ...
                <script>
                    function AddNoticeRow()
                    {
                        ...
                        tableBody.append(jQuery('<tr>')
                            .append(jQuery('<td>')
                                .append(jQuery('<input>').attr('type', 'text').attr('class', 'date-picker start-date').attr('placeholder', 'exemple: 2017-12-29')))
                            .append(jQuery('<td>')
                                .append(jQuery('<input>').attr('type', 'text').attr('class', 'date-picker end-date').attr('placeholder', 'exemple: 2017-12-29')))
                            .append(jQuery('<td>')
            ...
            <script type="text/javascript">
                ...
                var daraw = [	
                    ...
                    { "d1": "Prénom", 	 	"d2": "ele_prenom", 		"d3": "Mobile", 			"d4": "ele_numeromobile" },
                    { "d1": "Nom", 		 	"d2": "ele_nom", 			"d3": "Email", 				"d4": "ele_mail" },
                    { "d1": "Rue", 			"d2": "ele_rue", 			"d3": "Statut Juridique", 	"d4": "ele_majeur" },
                    { "d1": "NPA", 			"d2": "ele_npa", 			"d3": "Date de Naissance", 	"d4": "ele_datedenaissance" },
                    { "d1": "Localité",		"d2": "ele_localite", 		"d3": "-", 	"d4": "-" }
                ];
                ...
                var darbw = [	
					{ "d1": "Id Élève",	 		"d2": "id_eleve",	 	"d3": "Code Barre",		"d4": "id_codebarre" },
                    { "d1": "Élève numéro de contrat",	 		"d2": "ele_numerodecontrat", 			"d3": "Dérogation",	    "d4": "ele_derogation"  },
                    { "d1": "Désavantage", 		"d2": "ele_desavantage", 	"d3": "Élève Statut",	"d4": "ele_statut" }
                ];
				/***  Représentant Légal ***/
                var darcw = [	
					{ "d1": "Politesse", 	"d2": "rep_politesse", 	"d3": "",        			"d4": "" },
                    { "d1": "Prénom", 	 	"d2": "rep_prenom", 	"d3": "Nom", 		    	"d4": "rep_nom" },
                    { "d1": "Rue", 		 	"d2": "rep_Rue", 		"d3": "Téléphone 1", 		"d4": "rep_tel1" },
                    { "d1": "NPA", 	        "d2": "rep_npa", 		"d3": "Téléphone 2", 		"d4": "rep_tel2" },
                    { "d1": "Localité", 	"d2": "rep_localite", 	"d3": "Email", 	            "d4": "rep_mail" },
                    { "d1": "-", 			"d2": "-", 		        "d3": "Mobile", 	        "d4": "rep_numeromobile" }
                ...
                function getEdl( ta, va )
                {
                    if(el_data.hasOwnProperty(""+ta+"")){	//console.log(ta+" key found and its value is "+ el_data[ta]);
                        if (ta == 'ele_datedenaissance' )
						{
                            ...
                            var date = new Date( el_data[ta]);
                            var date_formatted = ('0' + date.getDate()).slice(-2)+ "." + ( '0' + date.getMonth()).slice(-2) + "."+date.getFullYear();
						    return '<input type="text" class="form-control date-picker naissance-date" name="'+ta+'"  id="'+ta+'" value="'+date_formatted+'" >';
						...
						if (ta == 'id_eleve')
                        {
                            return '<input type="text" class="form-control" name="'+ta+'" id="'+ta+'" value="'+el_data[ta]+'"  disabled="disabled" />'
                        }
                        ...
                        if (ta == 'ele_politesse' || ta == 'rep_politesse' )
                        {
                            ...
                            var majeur = "Majeur-e";
                            if ( el_data[ta] == "Majeur-e")
                                majeur = "<option value=\"Majeur-e\" selected=\"selected\" >Majeur-e</option><option value=\"Mineur-e\">Mineur-e</option>";
                            else if ( el_data[ta] == "Mineur-e")
                                majeur = "<option value=\"Majeur-e\" >Majeur-e</option><option value=\"Mineur-e\" selected=\"selected\" >Mineur-e</option>";
                            else
                                majeur = "<option value=\"Majeur-e\" >Majeur-e</option><option value=\"Mineur-e\">Mineur-e</option>";
