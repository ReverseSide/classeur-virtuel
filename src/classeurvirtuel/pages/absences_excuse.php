...
$requete5=$bdd->prepare("SELECT count(id_absence)as 'nbrperiode',id_eleve,ele_nom,ele_prenom,cla_nom,abs_date,abs_excuse,ele_mail,idx_entreprise FROM t_absence,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_absence.idx_eleve=t_eleve.id_eleve AND t_absence.idx_cours=t_sdh.id_sdh AND t_absence.idx_classe=t_classe.id_classe  AND t_sdh.cou_periode=t_horaire.id_horaire  AND abs_traite='0' and abs_excuse ='oui'  and t_absence.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') group by abs_date,idx_eleve ORDER BY abs_date desc ");
$requete5->execute();
$nbperiode=$requete5->fetchAll();


$requete4=$bdd->prepare("SELECT *	 FROM t_absence,t_eleve,t_sdh,t_classe,t_horaire  WHERE t_absence.idx_eleve=t_eleve.id_eleve AND t_absence.idx_cours=t_sdh.id_sdh AND t_absence.idx_classe=t_classe.id_classe AND t_sdh.cou_periode=t_horaire.id_horaire  AND abs_traite='0' and abs_excuse ='oui'  and t_absence.idx_classe in (select id_classe from t_classe where t_classe.idx_departement='".$_GET['dpt'] ."') ORDER BY abs_date desc");
$requete4->execute();
...
<div class="col-xs-12">
    <div class="col-lg-8 col-sm-8 col-xs-12 no-padding edusecArLangCss"><h3 class="box-title"><i class="fa fa-th-list"></i> Absences excusées </h3></div>
    <div class="col-lg-4 col-sm-4 col-xs-12 no-padding" style="padding-top: 20px !important;">
	  ...
	  <a class="btn btn-block btn-primary"  href="absences_nexc.php?dpt= <?php echo $_GET['dpt'];?>">Retour</a>    </div>
</div>
</div>
