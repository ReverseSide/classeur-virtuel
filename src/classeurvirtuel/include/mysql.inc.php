...
// *******************************************************************
    ...
    // Param.: 	(codebarre de l'élève)
    // *******************************************************************
    public function GetStudentOnly($id)
    {
	...
	private function SetAlternances($date, &$alternance, &$semestrePaire, &$semestre)
	{
		...
		$weeknumber=$this->datediff('ww', '21 August 2017', $textDate, false);
    ...
    // *******************************************************************
    // Nom :	GetEntreprise
    // But :	Retourne l'ensemble des valeurs d'une entreprise
    // Retour:	Tableau
    // Param.: 	(id entreprise)
    // *******************************************************************
    public function GetEntreprise($id)
    {
        $sth=$this->objConnexion->prepare("SELECT * FROM t_entreprise WHERE id_entreprise= :id");
    ...
    // *******************************************************************
    // Nom :	GetEntrepriseMaitre
    // But :	Retourne l'ensemble des maitres d'une entreprise
    // Retour:	Tableau
    // Param.: 	(id entreprise)
    // *******************************************************************
    public function GetEntrepriseMaitre($id)
    {
        $sth=$this->objConnexion->prepare("SELECT * FROM t_maitredapprentissage WHERE idx_entreprise= :id AND (mai_deactivate= 0 OR `mai_deactivate` IS NULL)");
        $sth->execute(array(':id'=>$id));
        $data = $sth->fetchAll();
        return $data;
    }//GetEntrepriseMaitre
    ...
    // *******************************************************************
    // Nom :	GetVocation
    // But :	Retourne l'ensemble des vacances
    // Retour:	Tableau
    // Param.: 	(codebarre de l'élève)
    // *******************************************************************
    public function GetVocation()
    {
        $strSelect="SELECT * FROM t_vacance;";
        $tabReponse=$this->Request($strSelect);
        return $tabReponse;
    }//GetVocation
    ...
    // *******************************************************************
    // Nom :	IsVocation
    // But :	Cherche si la date passé en paramètre correspond à une entrée de vacance dans la base de donnée
    // Retour:	Boolean
    // Param.: 	(date)
    // *******************************************************************
    public function IsVocation($date)
    {
        $response = false;
        $today=$date;
        $sth=$this->objConnexion->prepare("SELECT * FROM t_vacance WHERE debutDeVacances< :today AND finDeVacances>= :today");
    ...
    // *******************************************************************
    // Nom :	IsAdmin
    // But :	Cherche si la personne est admin
    // Retour:	Boolean
    // Param.: 	(user_id)
    // *******************************************************************
    ...
    {
        $response = false;
        $sth=$this->objConnexion->prepare("SELECT * FROM t_professeur WHERE id_professeur = :user_id and idx_departement = 8");
    ...
    // *******************************************************************
    // Nom :	UpdateMajeur
    // But :	Passe à majeur les élèves
    // Utilisé par : la cron journalière
    // Retour:	Boolean
    // Param.:
    // *******************************************************************
    public function UpdateMajeur()
    {
        $response = false;
        $sth=$this->objConnexion->prepare("UPDATE t_eleve SET ele_majeur=\"Majeur-e\" WHERE (round(DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), `ele_datedenaissance`)), \"%Y\"),0) >= 18 AND `ele_majeur` <> \"Majeur-e\")");
        $sth->execute();
        if ($sth->rowCount() > 0) {
            $response = true;
        }
    ...
    if ($this->IsVocation(date("Y-m-d", $date))==true){

        $strSelect="SELECT id_sdh, idx_classe, cou_duree, cou_heuredebut, cla_nom, cou_alternance, cou_matcode, cou_matlibelle FROM t_sdh, t_classe WHERE 0=1";
        $tabReponse=$this->Request($strSelect);
        return $tabReponse;
        ...
        {
                $strSelect="SELECT id_sdh, idx_classe, cou_duree, cou_heuredebut, cou_periode, cla_nom, cou_alternance, cou_matcode, cou_matlibelle,t_professeur.pro_nom,t_professeur.pro_prenom
    ...
    // Param.: 	(id de la classe)
    // *******************************************************************
    public function GetClassConge($class, $date = null)
    {
        if (is_null($date))
            $date = time();
        $today=date("Y-m-d", $date);

        $strSelect="SELECT id_absence, abs_periode, abs_excuse, idx_eleve, idx_cours, idx_classe FROM t_conge WHERE idx_classe='$class' AND abs_date='$today'";
        $tabReponse=$this->Request($strSelect);
        return $tabReponse;
    }

    // *******************************************************************
    // Nom :	GetClassMissing_te
    // But :	Va chercher les absences de la journnée d'une classe
    // Retour:	Tableau
    // Param.: 	(id de la classe)
    // *******************************************************************
    public function GetClassMissing_te($class, $date = null)
    {
        if (is_null($date))
            $date = time();
        $today=date("Y-m-d", $date);

        $strSelect="SELECT id_absence, abs_periode, abs_excuse, idx_eleve, idx_cours, idx_classe FROM t_absence_te WHERE idx_classe='$class' AND abs_date='$today'";
        $tabReponse=$this->Request($strSelect);
        return $tabReponse;
    }
    ...
    public function InsertParticuliar($id_eleve, $id_professeur, $period, $status, $id_classe, $idcours, $idcodebarre, $date = null)
    {
        ...
        $addED = function() use ($today, $period, $id_eleve, $id_professeur, $idcours, $id_classe, $idcodebarre)
        {
            $req = $this->objConnexion->prepare('INSERT INTO t_absence(abs_date, abs_periode, abs_excuse, idx_eleve, idx_professeur, idx_cours, idx_classe, idx_codebarre) VALUES(:date, :periode, \'ED\', :idx_eleve, :idx_professeur, :cours, :classe, :codebarre)');
            $req->execute(array(
                'date' => $today,
                'periode' => $period,
                'idx_eleve' => $id_eleve,
        ...
        $remED = function() use ($today, $id_eleve, $period)
        {
            $req = $this->objConnexion->prepare("DELETE FROM t_absence WHERE abs_date= :today AND idx_eleve= :ideleve AND abs_periode= :period");
            $req->execute(array(
                'today' => $today,
        ...
        $addEH = function() use ($today, $period, $id_eleve, $id_professeur, $idcours, $id_classe, $idcodebarre)
        {
            $req = $this->objConnexion->prepare('INSERT INTO t_absence(abs_date, abs_periode, abs_excuse, idx_eleve, idx_professeur, idx_cours, idx_classe, idx_codebarre) VALUES(:date, :periode, \'EH\', :idx_eleve, :idx_professeur, :cours, :classe, :codebarre)');
            $req->execute(array(
                'date' => $today,
                'periode' => $period,
                'idx_eleve' => $id_eleve,
        ...
        $remEH = function() use ($today, $id_eleve, $period)
        {
            $req = $this->objConnexion->prepare("DELETE FROM t_absence WHERE abs_date= :today AND idx_eleve= :ideleve AND abs_periode= :period");
            $req->execute(array(
                'today' => $today,
        ...
        $addTE = function() use ($today, $period, $id_eleve, $id_professeur, $idcours, $id_classe, $idcodebarre)
        {
            $req = $this->objConnexion->prepare('INSERT INTO t_absence_te(abs_date, abs_periode, abs_excuse, idx_eleve, idx_professeur, idx_cours, idx_classe, idx_codebarre) VALUES(:date, :periode, \'TE\', :idx_eleve, :idx_professeur, :cours, :classe, :codebarre)');
            $req->execute(array(
                'date' => $today,
                'periode' => $period,
                'idx_eleve' => $id_eleve,
        ...
        $remTE = function() use ($today, $id_eleve, $period)
        {
            $req = $this->objConnexion->prepare("DELETE FROM t_absence_te WHERE abs_date= :today AND idx_eleve= :ideleve AND abs_periode= :period");
            $req->execute(array(
                'today' => $today,
                'ideleve' => $id_eleve,
                'period' => $period
        ...
        $addCon = function() use ($today, $period, $id_eleve, $id_professeur, $idcours, $id_classe, $idcodebarre)
        {
            $req = $this->objConnexion->prepare('INSERT INTO t_conge(abs_date, abs_periode, abs_excuse, idx_eleve, idx_professeur, idx_cours, idx_classe, idx_codebarre) VALUES(:date, :periode, \'\', :idx_eleve, :idx_professeur, :cours, :classe, :codebarre)');
            $req->execute(array(
                'date' => $today,
                'periode' => $period,
                'idx_eleve' => $id_eleve,
        ...
        $remCon = function() use ($today, $id_eleve, $period)
        {
            $req = $this->objConnexion->prepare("DELETE FROM t_conge WHERE abs_date= :today AND idx_eleve= :ideleve AND abs_periode= :period");
            $req->execute(array(
                'today' => $today,
                'ideleve' => $id_eleve,
                'period' => $period
    ...
    // Param.: 	(id cours, date)
    // *******************************************************************
    public function CheckPresent($idProf, $idCours, $date)
    {
        $pdo = &$this->objConnexion;
        $strSelect="SELECT idx_professeur FROM t_presprof WHERE pre_date = ". $pdo->quote($date) ." AND idx_cours = ". $pdo->quote($idCours);
        $tabReponse=$this->Request($strSelect);
    ...
    public function GetClassDepartment($iddepartmeent)
    {
        ...
        $strSelect="SELECT * FROM t_classe WHERE idx_departement=$iddepartmeent ORDER BY t_classe.cla_nom ASC ";
        $tabReponse=$this->Request($strSelect);
        return $tabReponse;
    }//GetClassDepartment
...
// *******************************************************************
 ...
 // *******************************************************************
    ...
    {
        $id=$_SESSION['user_id'];
        $strSelect="SELECT * FROM t_classe LEFT JOIN t_maclasse ON t_maclasse.idx_classe1=t_classe.id_classe LEFT JOIN t_professeur ON t_classe.idx_maitre_classe = t_professeur.id_professeur WHERE t_maclasse.idx_professeur=$id";
